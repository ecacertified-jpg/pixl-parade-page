...
export default function Dashboard() {
  ...
  if (dashboardLoading && friends.length === 0 && events.length === 0) {
    ...
    <div className="min-h-screen bg-gradient-background">
      ...
      <main className="max-w-md mx-auto px-4 py-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TooltipProvider>
            <TabsList className="grid grid-cols-6 gap-0 bg-card/90 dark:bg-card/90 backdrop-blur-sm p-1 rounded-xl border border-border h-auto shadow-card">
              <Tooltip>
          ...
          <TabsContent value="amis" className="mt-4" forceMount={activeTab === 'amis' ? true : undefined} hidden={activeTab !== 'amis'}>
            {/* Birthday page sharing banner */}
            {!hasSharedBirthday && birthdayPageSlug && funds.some(f => f.occasion === 'birthday' && f.creator_id === user?.id && (f.status === 'active' || f.status === 'completed')) && (
