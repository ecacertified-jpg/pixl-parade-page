...
const BusinessAuth = () => {
  ...
  const { showFallback, defaultMethod, smsAvailable, autoWhatsApp } = useWhatsAppFallback(countryCode);
  ...
  const sendWhatsAppOtp = async (fullPhone: string, purpose: 'signin' | 'signup', metadata?: any) => {
    ...
    try {
      ...
      const response = await fetch(`${SUPABASE_URL}/functions/v1/send-whatsapp-otp`, {
        ...
        body: JSON.stringify({ phone: fullPhone, purpose, user_metadata: metadata }),
      });
  ...
  const handleSignUpSubmit = async (data: SignUpFormData) => {
    ...
    // Check if we need to show method selector first (CI, SN)
    if (showFallback && !otpMethod) {
      setPendingPhoneFormData(data);
      return;
    }
                  ...
                  <div className="mt-4">
                    ...
                    {authInputMethod === 'phone' ? (
                      <form onSubmit={signInForm.handleSubmit(sendOtpSignIn)} className="space-y-4">
                        ...
                        <WhatsAppAutoIndicator phonePrefix={countryCode} />
