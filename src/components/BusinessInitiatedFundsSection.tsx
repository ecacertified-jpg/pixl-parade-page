...
export function BusinessInitiatedFundsSection() {
  const { user } = useAuth();
  const { funds: businessFunds, loading } = useBusinessCollectiveFunds();
    ...
    try {
      ...
      for (const fund of businessFunds) {
        ...
        detailedFunds.push({
          ...
          beneficiary_name: fund.beneficiary 
            ? `${fund.beneficiary.first_name || ''} ${fund.beneficiary.last_name || ''}`.trim() || 'Bénéficiaire'
            : 'Bénéficiaire',
          beneficiary_phone: fund.beneficiary?.phone,
          beneficiary_address: '', // Would need to be added to profiles table
          contributors,
  ...
  if (fundsWithDetails.length === 0) {
    return (
      <div className="text-center py-8 text-muted-foreground">
        ...
        <p>Aucune cotisation initiée</p>
        <p className="text-sm">Les cotisations que vous créez pour vos clients apparaîtront ici</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {fundsWithDetails.map((fund) => {
        ...
        return (
          <Card key={fund.id} className="p-6 border border-border/50 hover:shadow-md transition-all duration-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {fund.contributors.length > 0 ? (
                  ...
                  <p className="text-sm text-muted-foreground">Aucun contributeur</p>
                )}
              </div>
              ...
              <div>
                ...
                {fund.non_contributors.length > 0 ? (
                  <div className="space-y-2 max-h-32 overflow-y-auto">
                    ...
                    {fund.non_contributors.length > 5 && (
                  ...
                  <p className="text-sm text-muted-foreground">Tous les amis ont contribué</p>
                )}
              </div>
            </div>

            {/* Actions si besoin */}
            {!isCompleted && fund.status === 'active' && (
