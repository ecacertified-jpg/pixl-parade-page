...
const fetchFriendRequests = async (userId: string): Promise<FriendRequestsData> => {
  const [{ data: received }, { data: sent }] = await Promise.all([
    ...
    supabase.from('contact_requests').select('*').eq('requester_id', userId).eq('status', 'pending'),
  ]);
...
export function useFriendRequests() {
  ...
  const sendRequest = useCallback(async (targetId: string, message?: string) => {
    ...
    try {
      ...
      const { error } = await supabase.from('contact_requests').insert({
        requester_id: user.id, target_id: targetId, message: message || null,
        status: 'pending', expires_at: expiresAt.toISOString(),
      });
      if (error) {
        if (error.code === '23505') toast.error('Une demande est déjà en cours pour cet utilisateur');
        else throw error;
        return false;
      }
  ...
  const acceptRequest = useCallback(async (requestId: string, requesterId: string) => {
    ...
    try {
      ...
      const { error: relError } = await supabase.from('contact_relationships').insert({
        user_a: userA, user_b: userB, can_see_funds: true, relationship_type: 'friend',
      });
      if (relError && relError.code !== '23505') console.error('Error creating relationship:', relError);
