...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const platformMobileMoneyPhone = platformSetting?.setting_value?.value || ''
    ...
    const { data: split, error: splitErr } = await supabaseAdmin
      ...
      .insert({
        ...
        vendor_wave_phone: businessAccount?.mobile_money_merchant_phone || null,
    ...
    console.log(`✅ Mobile Money payment split created: vendor=${vendorAmount} XOF, platform=${platformAmount} XOF`)
    ...
    console.error('❌ process-mobile-money-payment error:', error.message)
