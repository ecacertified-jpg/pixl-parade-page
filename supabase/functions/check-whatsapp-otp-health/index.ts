      ...
      try {
        ...
        if (adminError) {
          ...
          if (adminEmails.length > 0) {
            console.log(`📧 Sending OTP alert email to ${adminEmails.length} admin(s)...`);
              ...
              <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; margin: 0; padding: 20px; background-color: #f9fafb;">
                <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                  <div style="background: linear-gradient(135deg, #dc2626, #991b1b); padding: 24px; text-align: center;">
                    <h1 style="color: white; margin: 0; font-size: 22px;">🚨 Alerte OTP WhatsApp</h1>
                    <p style="color: rgba(255,255,255,0.9); margin: 8px 0 0 0; font-size: 14px;">Taux de succès critique détecté</p>
                  </div>
                  <div style="padding: 24px;">
                    <p style="color: #374151; font-size: 15px; margin: 0 0 16px 0;">
                      Le taux de succès OTP WhatsApp est passé sous le seuil critique. Voici les détails :
                    </p>
                    <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
                      <tr style="border-bottom: 1px solid #e5e7eb;">
                        <td style="padding: 10px 12px; font-weight: 600; color: #6b7280; font-size: 13px;">Taux de succès</td>
                        <td style="padding: 10px 12px; text-align: right; font-weight: 700; color: #dc2626; font-size: 16px;">${successRate}%</td>
                      </tr>
                      <tr style="border-bottom: 1px solid #e5e7eb;">
                      ...
                      <tr style="border-bottom: 1px solid #e5e7eb;">
                        <td style="padding: 10px 12px; font-weight: 600; color: #6b7280; font-size: 13px;">OTP envoyés</td>
                        <td style="padding: 10px 12px; text-align: right; font-size: 14px;">${totalSent}</td>
                      </tr>
                      <tr style="border-bottom: 1px solid #e5e7eb;">
                        <td style="padding: 10px 12px; font-weight: 600; color: #6b7280; font-size: 13px;">OTP vérifiés</td>
                        <td style="padding: 10px 12px; text-align: right; font-size: 14px;">${totalVerified}</td>
                      </tr>
                      <tr>
                    ...
                    <div style="text-align: center;">
                      <a href="${appUrl}/admin/whatsapp-otp" style="display: inline-block; background: #dc2626; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 14px;">
                        Voir le dashboard OTP
                      </a>
                    </div>
                  </div>
            ...
            const emailResponse = await resend.emails.send({
              ...
              to: adminEmails,
              subject: `🚨 [CRITIQUE] Alerte OTP WhatsApp - Taux de succès à ${successRate}%`,
              html: htmlContent,
            });

            report.email_sent = true;
            console.log('✅ Email OTP alert sent:', emailResponse);
          } else {
            console.log('⚠️ No admin emails found for OTP alert');
          }
        }
      } catch (emailError) {
        console.error('❌ Error sending OTP alert email (non-blocking):', emailError);
      }
    } else {
