-- Fix the find_users_in_delivery_zones function to use correct columns
...
BEGIN
  ...
  AND (
    -- Check if user's city matches any delivery zone
    ...
    OR
    -- If no city specified, include all users for now
    p.city IS NULL
  )
